...
export const useCalendar = (event, isEventHandlerSidebarActive, isLeftSidebarOpen) => {
  ...
  const extractEventDataFromEventApi = eventApi => {
    const { id, title, start, end, extendedProps: { calendar, client, city, mission_type }, allDay } = eventApi
  ...
  const addEvent = async _event => {
    try {
      const response = await store.addEvent(_event)
  ...
  const updateEvent = _event => {
    store.updateEvent(_event)
      .then(response => {
        ...
        const extendedPropsToUpdate = ['calendar', 'client', 'city', 'mission_type']
  ...
  const calendarOptions = {
    ...
    eventClick({ event: clickedEvent, jsEvent }) {
      // Prevent the URL from being interpreted as a path
