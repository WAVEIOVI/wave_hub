...
// 👉 Fetching addresses
const fetchSupplierAddresses = () => {
  supplierListStore.fetchSupplierAddresses({
...
const fetchSupplierAddress = addressId => {
  supplierListStore.fetchSupplierAddress(Number(route.params.id), addressId)
    .then(response => {
...
const addAddress = async addressData => {
  try {
    const response = await supplierListStore.addSupplierAddress(Number(route.params.id), addressData)
...
const updateAddress = async addressData => {
  ...
  try {
    ...
    const response = await supplierListStore.updateSupplierAddress(Number(route.params.id), addressId, addressData)
...
const deleteAddress = addressId => {
  supplierListStore.deleteSupplierAddress(Number(route.params.id), addressId)
    .then(response => {
...
watchEffect(fetchSupplierAddresses)
</script>
...
<template>
  ...
  <section>
    <VRow>
      <VCol cols="12">
        <VCard title="Supplier Addresses">
          <VDivider />
          <!-- SECTION - Table Tools -->
          ...
          >
            <!-- 👉 Address Title -->
            <template #[`item.su_address_title`]="{ item }">
              <div class="d-flex align-center">
                <VAvatar
                  ...
                  class="me-3"
                >
                  <span>{{ avatarText(item.raw.su_address_title) }}</span>
                </VAvatar>
                ...
                <div class="d-flex flex-column">
                  <h6 class="text-base">
                    ...
                    >
                      {{ item.raw.su_address_title }}
                    </RouterLink>
                  </h6>

                  <span class="text-sm text-medium-emphasis">#{{ item.raw.su_address_id }}</span>
                </div>
              </div>
            </template>
            <!-- 👉 Actions -->
            <template #[`item.actions`]="{ item }">
              <IconBtn 
                @click=" fetchSupplierAddress(item.raw.id),
