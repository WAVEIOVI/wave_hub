<script setup>
import AdditionalDetails from '@/views/contact/supplier/edit/AdditionalDetails.vue'
import BasicSupplierInformation from '@/views/contact/supplier/edit/BasicSupplierInformation.vue'
import SupplierAddressAndContacts from '@/views/contact/supplier/edit/SupplierAddressAndContacts.vue'
...
// 👉 Fetch Supplier
onMounted(() => {
  supplierListStore.fetchSupplier(Number(route.params.id)).then(response => {
    supplierData.value = response.data.data
  })
})

// 👉 Update Supplier
const updateSupplier = async  updatedSupplierData => {
  try {
    const response = await supplierListStore.updateSupplier(Number(route.params.id), updatedSupplierData)
